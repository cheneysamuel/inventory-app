...
async function insertBulkInventoryRecords(inventoryDataArray) {
    const results = [];
    const errors = [];
    console.log("insertBulkInventoryRecords called with inventoryDataArray:", inventoryDataArray);
    console.log('Inserting bulk inventory records:', inventoryDataArray);
    console.log('inventoryDataArray.length: ', inventoryDataArray.length);
...
async function generateBulkItemTypesTable() {
    ...
    for (const itemType of bulkItemTypes) {
        const row = document.createElement('tr');
        row.dataset.itemTypeId = itemType.id;

        // Await the aggregates
        const agg = await getBulkInventoryAggregates(itemType.id);
        ...
        quantityInput.addEventListener('focus', function() { this.select(); });
...
async function populateBulkSerializedDropdowns() {
    ...
    console.log('Serialized item types for dropdown:', itemTypes);
